...
print('\n测试9种方法...')
r1 = test_predict(predict_9methods, '9种方法', 5000)
print('9种方法: %.2f%% | 最大连中%d | 最大不中%d' % (r1['hit_rate'], r1['max_streak'], r1['max_miss']))

print('\n测试10种方法（+万能六码）...')
r2 = test_predict(predict_10methods, '10种方法', 5000)
print('10种方法: %.2f%% | 最大连中%d | 最大不中%d' % (r2['hit_rate'], r2['max_streak'], r2['max_miss']))

# 额外测试：万能六码覆盖率
print('\n测试万能六码覆盖率...')
covered = 0
total_test = min(5000, len(df) - 100)
for i in range(100, 100 + total_test):
    real = [int(df.iloc[i]['num1']), int(df.iloc[i]['num2']), int(df.iloc[i]['num3'])]
    real_set = set(real)
    for group in WAN_NENG_6:
        if real_set.issubset(set(group)):
            covered += 1
            break
print('万能六码覆盖率: %.2f%% (理论100%%)' % (covered/total_test*100))
...
print('|------|--------|------|---------|---------|')
print('| 9种方法 | %.2f%% | - | %d期 | %d期 |' % (r1['hit_rate'], r1['max_streak'], r1['max_miss']))
print('| 10种方法 | %.2f%% | %+.2f%% | %d期 | %d期 |' % (r2['hit_rate'], r2['hit_rate']-r1['hit_rate'], r2['max_streak'], r2['max_miss']))
print()
print('='*70)