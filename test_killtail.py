...
def predict_kill_tail(df_train):
    """
    和值尾杀号法：
    杀掉被杀的和尾，保留的和尾加分
    """
...
print('四招杀尾准确率: %.2f%% (%d/%d)' % (kill_correct/kill_total*100, kill_correct, kill_total))

# 测试每招单独准确率
print('\n各招单独准确率:')
for name, idx in [('百位加一', 0), ('个位直杀', 1), ('十位减一', 2), ('最大乘最小', 3)]:
    correct = 0
    for i in range(100, min(3100, len(df) - 600)):
        last_nums = [int(df.iloc[i-1]['num1']), int(df.iloc[i-1]['num2']), int(df.iloc[i-1]['num3'])]
        real_sum = int(df.iloc[i]['num1']) + int(df.iloc[i]['num2']) + int(df.iloc[i]['num3'])
        ...
        if real_tail != kill_tails[idx]:
            correct += 1
    
    print('%s: %.2f%%' % (name, correct/3000*100))
...
def predict_11methods_killtail(df_train):
    """10方法 + 和值尾杀号"""
    scores = {d: 0 for d in range(10)}
    ...
    # 和值尾杀号
    for d, s in predict_kill_tail(df_train).items():
        scores[d] += s
...
print('\n测试11种方法（+和值尾杀号）...')
r2 = test_predict(predict_11methods_killtail, '+和值尾杀号', 5000)
print('+和值尾杀号: %.2f%%' % r2)
