...
"""
彩票分析系统 V4.1 - 融入旋转矩阵
334断组 + 五五分解 + 两码进位和差 + 012路 + 奇偶 + 形态 + 和值尾 + 旋转矩阵缩水
命中率：14.14% + 旋转矩阵优化
"""
...
print('='*70)
print('彩票分析系统 V4.1 - 融入旋转矩阵')
print('='*70)

# ============================================================
# 优化后的旋转矩阵（命中率41%的四码组合）
# ============================================================
ROTATION_MATRIX = {
    'A': [4, 5, 6, 8],   # 41.20% - 最高
    'B': [2, 4, 5, 8],   # 41.12%
...
# 根据上期特征选择旋转矩阵标识
def select_rotation_label(last_nums):
    """根据上期号码特征选择旋转矩阵标识"""
    n1, n2, n3 = last_nums
    sum_val = n1 + n2 + n3
    sum_tail = sum_val % 10
    span = max(last_nums) - min(last_nums)
...
# 用旋转矩阵缩水号码
def filter_by_rotation(numbers, rotation_combo):
    """用旋转矩阵组合过滤号码"""
    filtered = []
    for num in numbers:
        digits = [int(d) for d in num]
        # 检查号码是否都在旋转矩阵组合中
        if all(d in rotation_combo for d in digits):
            filtered.append(num)
    return filtered
...
# 号码推荐 - 融入旋转矩阵
# ============================================================
def predict_numbers(df_train, count=5, use_rotation=True):
    """推荐号码 - 支持旋转矩阵缩水"""
    ...
    # 选择旋转矩阵
    rotation_label = select_rotation_label(last_nums)
    rotation_combo = ROTATION_MATRIX[rotation_label]
    ...
    # 用旋转矩阵缩水（可选）
    if use_rotation:
        filtered = filter_by_rotation(candidates, rotation_combo)
        # 如果缩水后数量不足，补充未缩水的号码
    ...
    return numbers, gold_dan, silver_dan, top5, rotation_label, rotation_combo
...
def main():
    ...
    numbers, gold_dan, silver_dan, top5, rotation_label, rotation_combo = predict_numbers(df)
    ...
    print()
    print('【旋转矩阵】标识%s → 组合[%s]' % (rotation_label, ''.join(map(str, rotation_combo))))
    print()
    print('推荐号码:')
